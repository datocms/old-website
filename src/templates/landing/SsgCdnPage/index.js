...
export default class SsgCdnPage extends React.Component {
  render() {
    const { data: { ssg, cdn, home, features, reviews } } = this.props;

    return (
      <div>
        <Helmet title={`CMS for ${ssg.name} on ${cdn.name} - Admin interface for ${ssg.name} sites - DatoCMS`} />
        <Hero
          title={`The best companion for ${ssg.name} sites on ${cdn.name}`}
          description={`DatoCMS is an API-based administrative area for your ${ssg.name} websites. Let your clients publish new content independently, host the site on ${cdn.name}.`}
        />
