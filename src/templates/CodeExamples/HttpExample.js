...
function httpRequestPicker(request, resource, filter) {
  if (filter) {
  ...
  } else if (request && request.method) {
    return (
      <HttpRequest
...
function renderExample(example, resource, filter=null) {
    ...
    <div className={b()}>
      ...
      <div className={b('snippet')}>
        <pre className="language-text">
          <code>
            {httpRequestPicker(request, resource, filter)}
            <Headers>
...
export default class HttpExample extends React.Component {
  render() {
    ...
    if (filter) {
      return renderExample({}, link, filter);
    }
