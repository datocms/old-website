...
class UseCasesPage extends React.Component {
  renderWebsite(differentHeight, website, i) {
    const tech = website.technologies.find(
      x => x.type.slug === 'static-generator' || x.type.slug === 'framework',
    );
          ...
          {tech && (
            <div className={b('website-technology')}>
              <img
                alt={tech.name}
                src={tech.logo.url}
                className={b('website-technology-logo')}
              />
  ...
  render() {
    ...
    const allWebsites = data.websites.edges
      .map(e => e.node)
      .filter(w => w.image);
